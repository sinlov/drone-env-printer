...
var dronePlugin *env_printer_plugin.Plugin

// GlobalBeforeAction
// do command Action before flag global.
func GlobalBeforeAction(c *cli.Context) error {
	isDebug := env_printer_plugin.IsBuildDebugOpen(c)
	if isDebug {
		drone_log.OpenDebug()
	}
	...
	p, err := env_printer_plugin.BindCliFlag(c, cliVersion, pkgJson.GetPackageJsonName(), drone)
	if err != nil {
		return err
	}
	dronePlugin = p
